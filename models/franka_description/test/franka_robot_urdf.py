...
from urdf_parser_py.urdf import Cylinder, Mesh, Sphere
...
class FrankaRobotUrdfTest(UrdfTestCase):
    ...
    def test_xyz_will_apply_fixed_offset_to_parent(self):
        xyz = [0, 1, 2]
        urdf = self.xacro(
            self.file, args="gazebo:=true xyz:='%s'" % " ".join(str(x) for x in xyz)
        )
    ...
    def test_rpy_will_apply_fixed_offset_to_parent(self):
        rpy = [3, 4, 5]
        urdf = self.xacro(
            self.file, args="gazebo:=true rpy:='%s'" % " ".join(str(x) for x in rpy)
        )
        joint = urdf.joint_map["world_joint"]
        ...
        for transmission in urdf.transmissions:
            if transmission.type == "franka_hw/FrankaModelInterface":
                self.assertEqual(
                    ...
                    "First joint in the FrankaModelInterface transmission must be the root of the chain, i.e. '$(arm_id)_joint1' not '%s'"
                    % transmission.joints[0].name,
                )
                self.assertEqual(
                    arm_id + "_joint8",
                    transmission.joints[1].name,
                    "Second joint in the FrankaModelInterface transmission must be the tip of the chain, i.e. '$(arm_id)_joint8' not '%s'"
                    % transmission.joints[1].name,
                )
                break
        else:
